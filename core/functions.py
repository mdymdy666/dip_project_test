...
class myThreshold():
    ...
    def get1DMaxEntropyThreshold(self, imgSrc):
        ...
        for Y in range(MinValue, MaxValue + 1):
            Amount += float(HistGram[Y][0])  # 像素总数

        for Y in range(MinValue, MaxValue + 1):
            HistGramD[Y] = float(HistGram[Y][0]) / Amount + 1e-17
...
def get_name(binary_img, horiBoundaryCoor, origin=None):
    ...
    # 名字区域高度
    (up, down) = horiBoundaryCoor
    box = np.int32([[coors[0][0], up], [coors[-1][1], up], [coors[-1][1], down], [coors[0][0], down]])
...
def get_gender_ethic(binary_img, horiBoundaryCoor, origin=None):
    ...
    if type(origin) == np.ndarray:
        for i in range(len(coors)):
            box = np.int32([[coors[i][0], up], [coors[i][1], up], [coors[i][1], down], [coors[i][0], down]])
            ...
            try:
                ...
                en_char = pytesseract.image_to_string(crop_img, 'eng', config='--psm 7')
...
def get_address(binary_img, horiBoundaryCoor, origin=None):
    ...
    if type(origin) == np.ndarray:
        ...
        try:
            ...
            text = pytesseract.image_to_string(crop_img, 'eng', config='--psm 6')
